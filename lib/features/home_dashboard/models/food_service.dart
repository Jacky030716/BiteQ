...
Future<List<FoodItem>> fetchFoodItemsForDateRange(DateTime start, DateTime end) async {
    ...
    for (var doc in mealTypesSnapshot.docs) {
      ...
      for (var food in foods) {
        allItems.add(FoodItem(
          ...
          dateScanned: _helpers.parseDateFromTimeString(food['time'], date) ?? date,
        ));
      }
    }
  }
...
Future<Map<String, double>> getNutritionForDateRange(DateTime start, DateTime end) async {
  final user = FirebaseAuth.instance.currentUser;
  if (user == null) return {};
